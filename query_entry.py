...
async def run():
    ...
    statement = """SELECT id, url, "data", created_at, processed_at FROM public.api_dump where id = 4318107 order by id asc limit 100000"""
    ...
    for (_id, url, data, created_at, processed_at) in values:
        open(f"temp/{_id}x.json", "wb").write(orjson.dumps(orjson.loads(data)["data"]["home"]["home_timeline_urt"]["instructions"][0]["entries"]))
