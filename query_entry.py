...
async def run():
    ...
    statement = """SELECT id, url, "data", created_at, processed_at FROM public.api_dump where id = 2853971 order by id asc limit 100000"""
    ...
    for (_id, url, data, created_at, processed_at) in values:
        open(f"temp/{_id}.json", "wb").write(data)
